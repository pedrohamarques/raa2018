...
\begin{document}
    ...
    %------------------------------------------------------
    \setmethodology{The step is represented in the figure on the left: the body is discretized in a finite number particles, which each point carries the information while moving through a fixed background mesh, transmitting the information to the nodes where the forces are calculated. Then, this new information is transmitted to the particles where their position and velocity are updated, repeating the process until the end of the simulation. Furthermore, it avoids mesh distortion, so it's suited for landslide simulation. The main goal is to simulate, as a first approximation, the soil as a linear elastic media, comparing the results to the ones found on the literature, which uses elastoplastic, or even viscoelastoplastic models. }
    %------------------------------------------------------
    \setmethodologyfigure{mpmsteps.png}{width=6cm}

    % RESULTS TEXT AND FIGURE
    %------------------------------------------------------
   \setresults{Validation of the numerical program, the simulation of the landslide using different types of soil and the parametric study, including how occurs the redistribution of stresses due to the abrupt decrease in strength of the surface, the change of potential and kinetic energies during the slides and the \textit{runoff} velocity of the particles along different simulations will be determined.}
